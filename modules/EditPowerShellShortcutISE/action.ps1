...
try {
    ...
    if ($targetPath -like "*powershell.exe" -and $arguments -match '-File\s+"([^"]+)"') {
        ...
        if (Test-Path $scriptPath) {
            ...
            # Start PowerShell ISE with the script's directory as the working directory
            Start-Process -FilePath "powershell_ise.exe" -ArgumentList $scriptPath -WorkingDirectory $scriptDirectory
        }
        else {
    ...
    # --- FALLBACK LOGIC START ---
        ...
        if (Test-Path -Path $targetPath -PathType Leaf) {
            ...
            Start-Process -FilePath "powershell_ise.exe" -ArgumentList $targetPath -WorkingDirectory $scriptDirectory
        }
        else {
